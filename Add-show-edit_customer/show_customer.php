...
if (isset($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('ลบข้อมูลเรียบร้อยแล้ว'); window.location='show_customer.php';</script>";
...
if (isset($_SESSION['username'])) {
    ...
    $username = $_SESSION['Emp_Name'] . " " . $_SESSION['Emp_Lastname'];
}
// ดึงข้อมูลลูกค้าจากฐานข้อมูล
...
<div class="user-container">
    ...
    <b class="add-link" onclick="window.location.href='logout.php';">Logout</b>
</div>

 
    <h2>ข้อมูลลูกค้า</h2>
    <p style="text-align: right;"class="add-link" onclick="window.location.href='add_customer.php';">เพิ่มข้อมูลลูกค้า</p>
    <table>
        <tr>
            <th>ID</th>
            <th>ชื่อ - สกุล</th>
        ...
        <?php while ($row = $result->fetch_assoc()) { ?>
            <tr>
                ...
                <td><button class="edit-btn" onclick="window.location.href='edit_customer.php?id=<?php echo $row['Customer_id']; ?>'">แก้ไข</button></td>
...
<script>
    function confirmDelete(id) {
        if (confirm("คุณต้องการลบข้อมูลนี้ใช่หรือไม่?")) {
            window.location.href = "show_customer.php?id=" + id;
        }
    }
</script>
